 ...
 */
export async function renewCertificateWithSecret(certReq: Omit<CertDnsRenewReq, 'token'>): Promise<CertificateData> {
    try {
        certReq.pushS3 = true
        const uid = localStorage.getItem('userId') ?? '';
        const appId = import.meta.env.VITE_CF_APP_ID;
        // Step 1: Get List of CloudflareDNS secrets for the current user
        const secretEntries = await SecretsService.getUserSecretEntriesByAppId(uid, appId)

        if (secretEntries.length < 1) {
            throw new Error(`No secrets configured for UserID: ${uid} AppId: ${appId}`);
        }
